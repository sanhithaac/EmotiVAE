"""
explore_latent_axis.py
----------------------
Uniformly sweep a single dimension of the latent space while keeping
all other dimensions at zero, then decode the samples to visualise what
each latent dimension captures.

Example
-------
    python explore_latent_axis.py --axis=0 --img_size=50 --latent_dim=50
"""
...
def explore_axis(axis_idx, IMG_SIZE, LATENT_DIM):
    """
    Sweep the *axis_idx*-th latent dimension uniformly in [-1.2, 1.2],
    decode, and save the resulting image grid.

    Parameters
    ----------
    axis_idx : int
        Index of the latent dimension to vary (0-based, up to LATENT_DIM).
    IMG_SIZE : int
    ...
    grid.save(f"outputs/axis/axis_{axis_idx}.png")


if __name__ == "__main__":
    import argparse

    parser = argparse.ArgumentParser(description="Explore a single latent axis")
    parser.add_argument(
        "--axis", help="Latent axis index (default 0)", default=0, type=int
    )
    parser.add_argument(
    ...
    explore_axis(axis_idx=cli.axis, IMG_SIZE=cli.img_size, LATENT_DIM=cli.latent_dim)
